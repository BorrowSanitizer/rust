...
/// with a valid set of `Hooks`, which is provided from across the FFI.
...
impl GlobalCtx {
    /// Creates a new instance of `GlobalCtx` using the given `Hooks`.
...
pub struct BVec<T>(Vec<T, AllocHooks>);

impl<T> Deref for BVec<T> {
    type Target = Vec<T, AllocHooks>;
...
#[derive(Debug, Clone)]
pub struct BVecDeque<T>(VecDeque<T, AllocHooks>);

impl<T> Deref for BVecDeque<T> {
    type Target = VecDeque<T, AllocHooks>;
...
#[derive(Debug, Clone)]
pub struct BHashMap<K, V>(HashMap<K, V, FxBuildHasher, AllocHooks>);

impl<K, V> Deref for BHashMap<K, V> {
    type Target = HashMap<K, V, FxBuildHasher, AllocHooks>;
    fn deref(&self) -> &Self::Target {
        &self.0
    }
}
...
/// `Hooks` to be valid.
#[inline]
pub unsafe fn init_global_ctx(hooks: Hooks) -> *mut GlobalCtx {
