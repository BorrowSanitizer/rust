...
enum RetagPlan {
    ...
    EmitRetag(RetagInfo, Option<AllocId>),
...
impl<'a, 'tcx> RetagPlan {
    fn emit_retag<Bx: BuilderMethods<'a, 'tcx>>(
        ...
        if matches!(ptr_kind, Some(Mutability::Mut) | None if !ty_is_unpin) {
            ...
            Some(RetagPlan::EmitRetag(
                RetagInfo { ptr_kind, size, ty_is_unpin, ty_is_freeze, is_protected },
                im_layout,
            ))
        }
    }
    fn for_value<Bx: BuilderMethods<'a, 'tcx>>(
        ...
        match layout.ty.kind() {
            &ty::Ref(_, _, mt) => Self::emit_retag(bx, fx, layout, Some(mt), retag_kind),
            &ty::RawPtr(_, _) => None,
            ty::Adt(adt, _) if adt.is_box() => Self::visit_box(bx, fx, layout, retag_kind),

            _ => None, // Self::walk_value(bx, fx, layout, kind),
        }
    }
...
impl<'a, 'tcx, Bx: BuilderMethods<'a, 'tcx>> FunctionCx<'a, 'tcx, Bx> {
    ...
    fn retag<R: Retagable<'a, 'tcx, Bx>>(&mut self, bx: &mut Bx, plan: RetagPlan, value: R) -> R {
    ...
    fn retag_inner<R: Retagable<'a, 'tcx, Bx>>(
        ...
        match plan {
            RetagPlan::EmitRetag(info, im_layout) => {
                ...
                let retagged_value = value.retag(bx, *info, im_layout);
...
trait Retagable<'a, 'tcx, Bx: BuilderMethods<'a, 'tcx>>: Copy {
    ...
    fn retag(&self, bx: &mut Bx, info: RetagInfo, im_layout: Bx::Value) -> Self;
    ...
    for OperandRef<'tcx, Bx::Value>
{
    ...
    fn retag(
        ...
        let retagged_val = bx.retag_reg(pointer, im_layout, info);
...
impl<'a, 'tcx, Bx: BuilderMethods<'a, 'tcx>> Retagable<'a, 'tcx, Bx> for PlaceRef<'tcx, Bx::Value> {
    ...
    fn retag(
        ...
        bx.retag_mem(self.val.llval, im_layout, info);
        *self
    }
