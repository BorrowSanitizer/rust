...
use crate::traits::{BaseTypeCodegenMethods, ConstCodegenMethods};
...
impl<'a, 'tcx, Bx: BuilderMethods<'a, 'tcx>> FunctionCx<'a, 'tcx, Bx> {
    ...
    fn retag_inner<R: Retagable<'a, 'tcx, Bx>>(
        ...
        match plan {
            RetagPlan::EmitRetag(size, perm, im_layout) => {
                ...
                let im_layout = if let Some(id) = im_layout {
                    bx.const_data_from_alloc(bx.tcx().global_alloc(*id).unwrap_memory())
                } else {
    ...
    fn im_span_alloc(&self, layout: TyAndLayout<'tcx>) -> Option<AllocId> {
        ...
        let c = self.cx.tcx().mk_const_alloc(alloc);
        Some(self.cx.tcx().reserve_and_set_memory_alloc(c))
    }
}
