            ...
            RetagPlan::Recurse { field_plans, variant_plans } => {
                ...
                if !variant_plans.is_empty() {
                    ...
                    if let Some(val) = bx.const_to_opt_u128(discr_val, false) {
                        ...
                        // The value of this operand will be different
                        // depending on which variant we have.
                        // To handle this, we create a temporary place.
                        let scratch = PlaceRef::alloca(bx, layout);

                        // Then, we store the operand to the place with all of
                        // the updates that we have collected so far.
                        let updated_op = builder.build(bx.cx());
                        scratch.storage_live(bx);
                        updated_op.store_with_annotation(bx, scratch);

                        // We retag the contents of the place using the `mem`
                        // variant of the retag.
                        self.retag_variants(bx, scratch, discr_val, variant_plans);

                        // Afterward, we load the now fully-updated operand and
                        // end the lifetime of the place.
                        let op = bx.load_operand(scratch);
                        scratch.storage_dead(bx);

                        // We replace our builder—otherwise, we would lose
                        // the updates we performed for each variant.
    ...
    fn retag_place(
        ...
        match plan {
            ...
            RetagPlan::Recurse { field_plans, variant_plans } => {
                ...
                if !variant_plans.is_empty() {
                    // The same process is used for `Rvalue::Discriminant`
