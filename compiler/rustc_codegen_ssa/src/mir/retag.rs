...
//!
//! There are two kinds of retag intrinsics (shown in LLVM IR).
//! ```
//! ptr __rust_retag_reg(ptr, i64, i8, ptr, ptr)
//! void __rust_retag_mem(ptr, i64, i8, ptr, ptr)
//! ```
//! The first variant is used when retagging an [`OperandRef`]. Its first argument is the pointer
//! being retagged, and it returns an alias with the same address, but different provenance. The
//! second variant also receives a pointer, but it points to a place where the pointer being retagged
//! is stored. We use this when retagging a [`PlaceRef`]. If we used the `reg` variant instead, then we
//! would need to load the pointer from the place and store the retagged result back to reflect that
//! its provenance had changed. However, if the place has LLVM's `readonly` attribute or equivalent,
//! then this additional store is undefined behavior. The `mem` variant communicates this level of indirection
//! without having to insert an explicit store. The remaining arguments to each form are the same, and are
//! provided by [`RetagInfo`]:
//!
...
//!
//! We attempt to retag every argument and return value of a function, and every rvalue of an assignment.
//! The first step to retagging is to generate a [`RetagPlan`], which describes which pointers within the
//! place or operand can be retagged. We traverse the [`RetagPlan`] to codegen each call, as needed.
//! Traversal is made easier by [`Retagable`]. Both [`PlaceRef`] and [`OperandRef`] implement this trait,
//! allowing us to use the same visitor pattern for each case.
...
impl<V> RetagPlan<V> {
    /// A helper function to project a [`RetagPlan`] for the given field.
...
impl<'a, 'tcx, V> RetagPlan<V> {
    ...
    fn walk_value<Bx: BuilderMethods<'a, 'tcx>>(
        ...
        let (fields, variants) = if let Some(def) = layout.ty.ty_adt_def()
            && def.is_unsafe_cell()
        {
            let field_layout = layout.field(bx, FieldIdx::ZERO.as_usize());
            let fields: Vec<(FieldIdx, RetagPlan<Bx::Value>)> =
                Self::build(bx, fx, field_layout, is_fn_entry)
    ...
    fn visit_box<Bx: BuilderMethods<'a, 'tcx>>(
        ...
        if ptr_layout.ty.is_box_global(bx.tcx()) {
            let pointee_ty = ptr_layout.ty.expect_boxed_ty();
            let pointee_layout = bx.layout_of(pointee_ty);
            if let Some(mut plan) = Self::emit_retag(bx, pointee_layout, None, is_fn_entry) {
                // `Unique<T>`
...
trait PerByteTracking<'a, 'tcx, Bx: BuilderMethods<'a, 'tcx>> {
    /// The range containing this type can be excluded.
...
impl RangeCollector {
    ...
    fn advance(&mut self, offset: Size) {
        if self.cursor + self.acc_offset != offset {
