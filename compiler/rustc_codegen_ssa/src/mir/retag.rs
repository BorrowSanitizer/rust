...
use rustc_middle::mir::{HasLocalDecls, Local, Place, RetagKind};
...
/// We emit retags after each assignment statement if the value being
/// assigned contains a reference. The target of an assignment is either
/// a place or a local.
pub(crate) enum RetagTarget<'tcx, V> {
    Place(PlaceRef<'tcx, V>),
    Operand(Local, OperandRef<'tcx, V>),
}

pub(crate) fn place_needs_retag<'tcx>(
    decls: &impl HasLocalDecls<'tcx>,
    place: &Place<'tcx>,
) -> bool {
    return !place.is_indirect_first_projection()
        && !decls.local_decls()[place.local].is_deref_temp();
}
...
enum RetagPlan {
    ...
    Recurse(IndexMap<FieldIdx, RetagPlan>, IndexMap<VariantIdx, RetagPlan>),
}

/// The information required to retag a pointer.
#[derive(Debug)]
struct RetagInfo {
    /// The permission(s) created by the retag.
    perm: u64,
    /// The `size` of the range within the allocation that can be accessed
    /// using the permission created by the retag.
    size: Size,
    /// An identifier for a global constant array of pairs of `u64` that describe
    /// interior mutable subranges within the range covered by the retag.
    im_layout: Option<AllocId>,
}
...
impl<'a, 'tcx> RetagPlan {
    fn for_value<Bx: BuilderMethods<'a, 'tcx>>(
        ...
        let emit_retag = |pointee_ty: Ty<'tcx>| {
            let pointee_layout = bx.layout_of(pointee_ty);
            if let Some(perm) =
                bx.tcx().retag_perm((bx.typing_env(), layout.ty, pointee_layout.ty, kind))
            {
                let im_layout = fx.im_span_alloc(pointee_layout);
                let size = pointee_layout.size;
                Some(RetagPlan::EmitRetag(RetagInfo { perm, size, im_layout }))
            } else {
                None
            }
        };
        ...
        match layout.ty.kind() {
            &ty::Ref(_, pointee_ty, _) => emit_retag(pointee_ty),
            &ty::RawPtr(pointee_ty, _) => emit_retag(pointee_ty),
            ty::Adt(adt, _) if adt.is_box() => Self::visit_box(bx, fx, layout, kind),

            _ => Self::walk_value(bx, fx, layout, kind),
        }
    }
    ...
    fn walk_value<Bx: BuilderMethods<'a, 'tcx>>(
            ...
            && def.is_unsafe_cell()
        {
            ...
            let fields: Vec<(FieldIdx, RetagPlan)> = Self::for_value(bx, fx, field_layout, kind)
    ...
    fn visit_box<Bx: BuilderMethods<'a, 'tcx>>(
        ...
        if layout.ty.is_box_global(bx.tcx()) {
                ...
                bx.tcx().retag_perm((bx.typing_env(), layout.ty, pointee_layout.ty, kind))
            {
                let size = pointee_layout.size;
                let im_layout = fx.im_span_alloc(pointee_layout);
                let leaf = RetagPlan::EmitRetag(RetagInfo { perm, size, im_layout });
                fields.push((
                    FieldIdx::ZERO,
...
impl RetagFlags {
    pub fn for_retag_kind(kind: RetagKind) -> Self {
        let mut flags = RetagFlags::empty();
        if kind == RetagKind::FnEntry {
            flags = flags | RetagFlags::FN_ENTRY;
        }
        flags
    }
    pub fn for_place<'tcx>(ctx: &impl HasLocalDecls<'tcx>, place: &Place<'tcx>) -> Self {
        let mut flags = RetagFlags::empty();
        if place.is_indirect_first_projection()
            && let Some(local) = ctx.local_decls().get(place.local)
            && local.ty.is_raw_ptr()
        {
            flags = flags | RetagFlags::FROM_RAW;
        }
        flags
    }
}
...
impl<'a, 'tcx, Bx: BuilderMethods<'a, 'tcx>> FunctionCx<'a, 'tcx, Bx> {
    pub(crate) fn codegen_retag_operand(
        ...
        if let OperandValue::Ref(place_ref) = op.val {
            ...
            op
        } else if let Some(plan) = RetagPlan::for_value(bx, self, op.layout, kind) {
            let flags = flags | RetagFlags::for_retag_kind(kind);
            self.retag(bx, flags, plan, op)
        } else {
            op
        }
    }
    ...
    pub(crate) fn codegen_retag_place(
        ...
        if let Some(plan) = RetagPlan::for_value(bx, self, place_ref.layout, kind) {
            let flags = flags | RetagFlags::for_retag_kind(kind);
            self.retag(bx, flags, plan, place_ref);
        }
    }
    ...
    fn retag<R: Retagable<'a, 'tcx, Bx>>(
        ...
        self.retag_inner(bx, &mut vcx, flags, &plan, value, Size::ZERO);
        vcx.resolve(bx)
    }
    ...
    fn retag_inner<R: Retagable<'a, 'tcx, Bx>>(
        ...
        match plan {
            RetagPlan::EmitRetag(RetagInfo { size, perm, im_layout }) => {
                let im_layout = if let Some(alloc_id) = im_layout {
                    let global_alloc = bx.tcx().global_alloc(*alloc_id);
                    let (_, align) = global_alloc.size_and_align(bx.tcx(), bx.typing_env());
                    let const_alloc = global_alloc.unwrap_memory();
                    let const_value = bx.cx().const_data_from_alloc(const_alloc);
                    bx.cx().static_addr_of(const_value, align, Some("retag"))
                } else {
                    bx.const_null(bx.type_ptr())
                };
                let retagged_value = value.retag(bx, *size, *perm, flags, im_layout);
            ...
            RetagPlan::Recurse(fields, variants) => {
                for (ix, field_plan) in fields.iter() {
                    ...
                    self.retag_inner(bx, vcx, flags, field_plan, field_value, field_cursor);
                }
                ...
                if !variants.is_empty() {
                    ...
                    if let Some(val) = bx.const_to_opt_u128(discr_val, false) {
                        ...
                        if let Some(variant_plan) = variants.get(&ix) {
                            self.retag_inner(bx, vcx, flags, variant_plan, variant_value, cursor);
                        }
                    } else {
    ...
    for OperandRef<'tcx, Bx::Value>
{
    ...
    fn retag(
        ...
        let retagged_val = bx.retag_operand(pointer, im_layout, size, perm, flags);
...
impl<'a, 'tcx, Bx: BuilderMethods<'a, 'tcx>> Retagable<'a, 'tcx, Bx> for PlaceRef<'tcx, Bx::Value> {
    ...
    fn retag(
        ...
        bx.retag_place(self.val.llval, im_layout, size, perm, flags);
        *self
    }
