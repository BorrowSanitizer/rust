...
impl<'a, 'tcx> RetagPlan {
        ...
        match layout.ty.kind() {
            &ty::Ref(_, pointee_ty, _) => {
                ...
                if let Some(perm) = bx.tcx().retag_perm((
                    ...
                    Some(RetagPlan::EmitRetag(pointee_layout.size, perm))
                } else {
                    None
                }
            }
    ...
    fn walk_value<Bx: BuilderMethods<'a, 'tcx>>(
        ...
        let (fields, variants) = if let Some(def) = layout.ty.ty_adt_def()
            && def.is_unsafe_cell()
        {
            let fields = Self::visit_field(bx, fx, layout, FieldIdx::from_usize(0), kind, true)
                .into_iter()
                .collect();
            (fields, Vec::new())
        } else {
            // A type can have both fields and variants.
            let fields: Vec<(FieldIdx, RetagPlan)> = match &layout.fields {
                FieldsShape::Union(_) => Vec::new(),
                FieldsShape::Primitive => Vec::new(),
                FieldsShape::Arbitrary { memory_index, .. } => memory_index
                    .indices()
                    .filter_map(|idx| Self::visit_field(bx, fx, layout, idx, kind, in_unsafe_cell))
                    .collect(),
        ...
        if fields.len() > 0 || variants.len() > 0 {
            Some(RetagPlan::Branch(RetagLayout {
    ...
    fn visit_box<Bx: BuilderMethods<'a, 'tcx>>(
        ...
        if let Some(field_plan) =
            Self::visit_field(bx, fx, layout, FieldIdx::ONE, kind, in_unsafe_cell)
        {
            fields.push(field_plan)
        }

        if fields.len() > 0 {
            let mut field_plan = RetagLayout::default();
            field_plan.fields = fields.into_iter().collect();
            Some(RetagPlan::Branch(field_plan))
        } else {
            None
        }
    }

    fn visit_field<Bx: BuilderMethods<'a, 'tcx>>(
        bx: &mut Bx,
        fx: &mut FunctionCx<'a, 'tcx, Bx>,
        layout: TyAndLayout<'tcx>,
        idx: FieldIdx,
        kind: RetagKind,
        in_unsafe_cell: bool,
    ) -> Option<(FieldIdx, RetagPlan)> {
        let field_layout = layout.field(bx.cx(), idx.as_usize());
        Self::for_value(bx, fx, field_layout, kind, in_unsafe_cell).map(|plan| (idx, plan))
    }
}
...
impl<'a, 'tcx, Bx: BuilderMethods<'a, 'tcx>> FunctionCx<'a, 'tcx, Bx> {
    ...
    fn retag_inner<R: Retagable<'a, 'tcx, Bx>>(
        ...
        match plan {
            ...
            RetagPlan::Branch(RetagLayout { fields, variants }) => {
                ...
                if variants.len() > 0 {
                    ...
                    if let Some(val) = bx.const_to_opt_u128(discr_val, false) {
                        ...
                        let mut blocks: Vec<(u128, Bx::BasicBlock)> = vec![];
                        ...
                        for (ix, plan) in variants.iter() {
                            ...
                            blocks.push((discr_val, variant_block))
                        }

                        bx.switch_to_block(root_block);
                        bx.switch(discr_val, terminator_block, blocks.drain(..));
                        bx.switch_to_block(terminator_block);

                        vcx.phi(value.layout(), bx, updates);
                    }
                }
            }
        }
    ...
    Sized + Clone
{
    fn phi(
        ...
        layout: TyAndLayout<'tcx>,
        bx: &mut Bx,
        branches: Vec<(Bx::BasicBlock, Self)>,
    );
    ...
    for ()
{
    ...
    fn phi(
        ...
        _bx: &mut Bx,
        _branches: Vec<(Bx::BasicBlock, Self)>,
    ) {
    }
    ...
    for OperandValueCx<Bx::Value>
{
    fn phi(
        ...
        layout: TyAndLayout<'tcx>,
        bx: &mut Bx,
        mut branches: Vec<(Bx::BasicBlock, Self)>,
    ) {
