...
impl<'a, 'tcx, V> RetagPlan<V> {
    ...
    /// Creates a [`RetagPlan::EmitRetag`] for a pointer with the given layout.
    ...
    /// which idnciate the offset and width of a range within the layout of a type. Returns a null
    ...
    fn alloc_ranges<Bx: BuilderMethods<'a, 'tcx>>(
        ...
        let tcx = bx.tcx();
        // The empty range is a null pointer.
        if ranges.is_empty() {
        ...
        let const_alloc = bx.tcx().mk_const_alloc(alloc);
...
trait PerByteTracking<'a, 'tcx, Bx: BuilderMethods<'a, 'tcx>> {
    ...
    /// This implies that [`PerByteTracking::contains`] is false for every
    /// field or variant without having to recurse any further.
...
impl RangeCollector {
    ...
    /// Move the collector forward, recording the current range if
    /// this leaves a gap between the start of the last range and the
    /// current offset.
...
/// require loading it, performing the retag, and then storing the retagged pointer back where it came from. This final
