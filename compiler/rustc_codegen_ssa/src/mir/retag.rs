...
impl<'a, 'tcx, V> RetagPlan<V> {
    fn emit_retag<Bx: BuilderMethods<'a, 'tcx>>(
        ...
        let ty_is_unpin: bool = pointee_ty.is_unpin(bx.tcx(), bx.typing_env());
        ...
        if matches!(ptr_kind, Some(Mutability::Mut) | None if !ty_is_unpin) {
            ...
            {
                // If everything is pinned, (e.g. `&mut UnsafePinned<T>`), then skip retags.
                return None;
            }
        }

        let pin_layout_ptr = Self::alloc_ranges(bx, pin_ranges);
        ...
        let im_layout_ptr = Self::alloc_ranges(bx, im_ranges);
