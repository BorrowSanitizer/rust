...
impl<'a, 'tcx, Bx: BuilderMethods<'a, 'tcx>> FunctionCx<'a, 'tcx, Bx> {
    pub(crate) fn codegen_tag_assignment(
        ...
        if bx.tcx().sess.opts.unstable_opts.codegen_emit_retag {
            if let Some(retag_kind) = retag_kind {
                self.codegen_retag_operand(bx, op, index, retag_kind);
    ...
    pub(crate) fn codegen_tag_assignment_for_box(
        ...
        if box_ty.is_box_global(bx.tcx()) {
            if let Some(kind) = kind {
                ...
                {
                    self.tagged_locals.insert(index);
                    operand.retag(bx, pointee_layout, index, perm, kind == RetagKind::FnEntry);
                }
            }
        }
    }
    ...
    pub(crate) fn codegen_retag(&mut self, bx: &mut Bx, place: &Place<'tcx>, kind: RetagKind) {
        ...
        // operand
        {
            ...
            RetagCx::visit(self, bx, kind, index, place);
        };
    }

    fn codegen_retag_operand(
        &mut self,
        bx: &mut Bx,
        op: OperandRef<'tcx, Bx::Value>,
        index: Local,
        ...
        if let OperandValue::Ref(place) = op.val {
            RetagCx::visit(self, bx, retag_kind, index, place.with_type(op.layout));
        } else {
            RetagCx::visit(self, bx, retag_kind, index, op);
        }
    }
}
