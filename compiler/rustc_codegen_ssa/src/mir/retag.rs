...
use crate::mir::{LocalRef, TyAndLayout};
use crate::traits::{BaseTypeCodegenMethods, ConstCodegenMethods, StaticCodegenMethods};
...
impl<'a, 'tcx> RetagPlan {
    fn for_value<Bx: BuilderMethods<'a, 'tcx>>(
        ...
        match layout.ty.kind() {
            &ty::Ref(_, pointee_ty, m) => {
                let pointee_layout = bx.layout_of(pointee_ty);
                bx.tcx().retag_perm((bx.typing_env(), pointee_layout.ty, Some(m), kind)).map(
                    |perm| {
                        RetagPlan::EmitRetag(
                            pointee_layout.size,
                            perm,
                            fx.im_span_alloc(pointee_layout),
                        )
                    },
                )
            }
    ...
    fn visit_box<Bx: BuilderMethods<'a, 'tcx>>(
        ...
        if layout.ty.is_box_global(bx.tcx()) {
            ...
            if let Some(perm) =
                bx.tcx().retag_perm((bx.typing_env(), pointee_layout.ty, None, kind))
            {
                let leaf = RetagPlan::EmitRetag(
                    pointee_layout.size,
                    perm,
                    fx.im_span_alloc(pointee_layout),
                );
...
impl<'a, 'tcx, Bx: BuilderMethods<'a, 'tcx>> FunctionCx<'a, 'tcx, Bx> {
    ...
    pub(crate) fn codegen_retag(&mut self, bx: &mut Bx, place: &Place<'tcx>, kind: RetagKind) {
        ...
        if let Some(plan) = RetagPlan::for_value(bx, self, layout, kind) {
    ...
    fn retag_inner<R: Retagable<'a, 'tcx, Bx>>(
        ...
        match plan {
            RetagPlan::EmitRetag(size, perm, im_layout) => {
                let is_fn_entry = kind == RetagKind::FnEntry;
                ...
                let retagged_value = value.retag(bx, *size, *perm, is_fn_entry, im_layout);
    ...
    for OperandValueCx<Bx::Value>
{
    ...
    fn resolve(&self, mut template: OperandRef<'tcx, Bx::Value>) -> OperandRef<'tcx, Bx::Value> {
        template.val = (*self).into();
        template
    }
    ...
    for OperandRef<'tcx, Bx::Value>
{
    ...
    fn retag(
        ...
        let retagged_val = bx.retag(pointer, size, perm, fn_entry, im_layout);
...
impl<'a, 'tcx, Bx: BuilderMethods<'a, 'tcx>> Retagable<'a, 'tcx, Bx> for PlaceRef<'tcx, Bx::Value> {
    ...
    fn retag(
        ...
        let pointer_to_retag = bx.load_from_place(bx.type_ptr(), self.val);
        let retagged_pointer = bx.retag(pointer_to_retag, size, perm, fn_entry, im_layout);
        bx.store_to_place(retagged_pointer, self.val);
        *self
    }
