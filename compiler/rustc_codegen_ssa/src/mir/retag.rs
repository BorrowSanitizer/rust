...
impl<'a, 'tcx, V> RetagPlan<V> {
    ...
    fn emit_retag<Bx: BuilderMethods<'a, 'tcx>>(
        ...
        if is_mutable {
            ...
            // If the entire type is `UnsafePinned`, then retagging is a noop.
            let everything_is_pinned = pinned_size == pointee_layout.size;
            // If a type does not contain `UnsafePinned`, but it is still `!Unpin`,
            // then we also skip retagging, since the type contains `PhantomPinned`.
            // We do not have a way to check for it explicitly until `PhantomPinned`
            // becomes a lang item.
            let contains_phantom_pinned =
                pin_ranges.is_empty() && !UnsafePinnedRanges::excludes(bx, pointee_ty);
            if everything_is_pinned || contains_phantom_pinned {
                return None;
            }
        }
