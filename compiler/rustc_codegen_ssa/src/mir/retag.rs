...
impl<'a, 'tcx, V> RetagPlan<V> {
    ...
    fn emit_retag<Bx: BuilderMethods<'a, 'tcx>>(
        ...
        // At the moment, `&mut !Unpin` does not get `noalias` nor `dereferenceable`,
        // so we also don't add protectors.
        let is_protected = is_fn_entry && (!is_mutable || pin_ranges.is_empty());
