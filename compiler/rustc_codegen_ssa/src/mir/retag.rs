...
impl<'a, 'tcx, V> RetagPlan<V> {
    ...
    fn emit_retag<Bx: BuilderMethods<'a, 'tcx>>(
        ...
        if is_mutable {
            let pinned_size = pin_ranges.first().and_then(|elem| elem.get(1)).copied();
            // If the entire type is `UnsafePinned`.
            let all_pinned = pinned_size == Some(pointee_layout.size);
            // We can't find any `UnsafePinned`, but the type is still
            // `!Unpin` or `!UnsafeUnpin`.
            let implicitly_pinned =
                pin_ranges.is_empty() && !UnsafePinnedRanges::excludes(bx, pointee_ty);
                ...
                if !variants.is_empty() {
                    ...
                    if let Some(val) = bx.const_to_opt_u128(discr_val, false) {
                        ...
                        for (ix, variant_plan) in variants.iter() {
                            ...
                            variant_edges.push((discr_val, variant_block))
                        }

                        bx.switch_to_block(root_block);
                        if let Some((variant_discr, variant_block)) = variant_edges.first()
                            && variant_edges.len() == 1
                        {
                            let variant_discr_ty = bx.backend_type(bx.layout_of(discr_ty));
                            let variant_discr_val =
                                bx.cx().const_uint_big(variant_discr_ty, *variant_discr);
                            let eq = bx.icmp(IntPredicate::IntEQ, discr_val, variant_discr_val);
                            bx.cond_br(eq, *variant_block, terminator_block);
                        } else {
