...
impl<'a, 'tcx, V> RetagPlan<V> {
    ...
    fn visit_box<Bx: BuilderMethods<'a, 'tcx>>(
        ...
        if layout.ty.is_box_global(bx.tcx()) {
            ...
            if let Some(plan) = Self::emit_retag(bx, pointee_layout, None, is_protected) {
                fields.push((
                    FieldIdx::ZERO,
    ...
    fn emit_retag<Bx: BuilderMethods<'a, 'tcx>>(
        ...
        // Note: even though we track the ranges in a type that are covered by
        // `UnsafeCell`, we also need an explicit flag indicate when a ZST
        // is `!Freeze`. The same goes for `Unpin`.
        let pointee_is_unpin: bool = pointee_ty.is_unpin(bx.tcx(), bx.typing_env())
            && pointee_ty.is_unsafe_unpin(bx.tcx(), bx.typing_env());

        let pointee_is_freeze: bool = pointee_ty.is_freeze(bx.tcx(), bx.typing_env());
        ...
        if matches!(ptr_kind, Some(Mutability::Mut) | None if !pointee_is_unpin) {
            ...
            {
                // If everything is `UnsafePinned`, then skip retagging.
                return None;
            }
        }
...
trait PerByteTracking<'a, 'tcx, Bx: BuilderMethods<'a, 'tcx>> {
    ...
    /// `None` implies `!contains`, while `Some(false)` implies `contains` only for types where we
    /// lack precision, like dynamic trait objects. This is necessary to handle the distinction between
    /// `Unpin` and `Freeze`, where `Freeze` types cannot contain `UnsafeCell` fields, but `Unpin` types can
    /// contain `UnsafePinned` fields.
    fn excludes(bx: &mut Bx, ty: Ty<'tcx>) -> Option<bool>;
    ...
    fn visit_layout(
        ...
        let excludes = Self::excludes(bx, layout.ty);

        if let Some(true) = excludes {
            return;
        }

        let include_imprecise = excludes == Some(false);
        ...
        if include_imprecise && imprecise {
        ...
        if Self::contains(bx, layout.ty) {
            collector.extend(layout.size);
        } else if union_or_primitive || has_multiple_variants {
            if include_imprecise {
                collector.extend(layout.size);
            }
        } else {
            let indices: Vec<FieldIdx> = match &layout.fields {
...
impl<'a, 'tcx, Bx: BuilderMethods<'a, 'tcx>> PerByteTracking<'a, 'tcx, Bx> for UnsafeCellRanges {
    fn excludes(bx: &mut Bx, ty: Ty<'tcx>) -> Option<bool> {
        ...
        Some(ty.is_freeze(bx.tcx(), bx.cx().typing_env()))
    }
...
impl<'a, 'tcx, Bx: BuilderMethods<'a, 'tcx>> PerByteTracking<'a, 'tcx, Bx> for UnsafePinnedRanges {
    fn excludes(_bx: &mut Bx, _ty: Ty<'tcx>) -> Option<bool> {
        // If a type is `Unpin`, then it might still contain `UnsafePinned`.
        None
    }
