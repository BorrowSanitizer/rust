...
fn arg_local_refs<'a, 'tcx, Bx: BuilderMethods<'a, 'tcx>>(
    ...
    if bx.tcx().sess.opts.unstable_opts.codegen_emit_retag {
        let inst = fx.instance;
        let codegen_fn_attrs = bx.tcx().codegen_fn_attrs(inst.def_id());

        let retag_kind = match (codegen_fn_attrs.inline, &codegen_fn_attrs.optimize) {
            // `optnone` requires `noinline`
            (_, OptimizeAttr::DoNotOptimize) => RetagKind::FnEntry,
            (InlineAttr::Never, _) => RetagKind::FnEntry,
            (InlineAttr::None, _) if inst.def.requires_inline(bx.tcx()) => RetagKind::Default,
            (InlineAttr::None, _) => RetagKind::FnEntry,
            (InlineAttr::Hint | InlineAttr::Always | InlineAttr::Force { .. }, _) => {
                RetagKind::Default
            }
        };
