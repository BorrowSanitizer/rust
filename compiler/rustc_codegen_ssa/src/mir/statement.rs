use rustc_middle::mir::{self, NonDivergingIntrinsic, Rvalue, StmtDebugInfo, WithRetag};
        ...
        match statement.kind {
            mir::StatementKind::Assign(box (ref place, ref rvalue)) => {
                let needs_retag = bx.tcx().sess.opts.unstable_opts.codegen_emit_retag.is_some()
                    ...
                    // `Ref` has its own internal retagging, and we want to skip intermediate copies.
                    || matches!(rvalue, Rvalue::Ref(..) | Rvalue::Use(_, WithRetag::No)));
