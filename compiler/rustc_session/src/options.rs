...
options! {
    ...
    codegen_retag_fields: RetagFields = (RetagFields::All, parse_retag_fields, [TRACKED],
        "controls whether retagging recurses into fields.
        `all` means it always recurses.
        `none` means it never recurses, which is unsound.
        `scalar` means it only recurses for types that are noalias in LLVM IR.
        (default: all)"),
    ...
    mir_emit_retag: Option<RetagMode> = (None, parse_retag_mode, [TRACKED],
        ...
        (default: no)"),
    mir_emit_retag_raw_ptr: bool = (true, parse_bool, [TRACKED],
