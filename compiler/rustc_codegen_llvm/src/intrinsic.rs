...
impl<'ll, 'tcx> IntrinsicCallBuilderMethods<'tcx> for Builder<'_, 'll, 'tcx> {
    ...
    fn link_tag(&mut self, pointer: Self::Value, local: Local) {
        let index = self.const_u32(local.as_u32());
        self.call_intrinsic("__bsan_link", &[self.type_ptr(), self.type_i32()], &[pointer, index]);
    }

    fn consume_tag(&mut self, pointer: Self::Value, local: Local) {
        let index = self.const_u32(local.as_u32());
        self.call_intrinsic(
            "__bsan_consume",
